...
pub fn handler(
    ...
    let FeeExtract {
        fees_extracted,
        amount_remaining,
    } = AssetSwapConfig::new(ctx.accounts.usdc_pair.redeem_fee)?.apply_fee(amount_in)?;
    require!(amount_remaining > UFix64::zero(), CoreError::ZeroAmount);
    require!(
        amount_remaining <= ctx.accounts.usdc_pair.virtual_stablecoin.supply()?,
    ...
    let vault_auth_bump = [ctx.accounts.usdc_pair.vault_auth_bump];
